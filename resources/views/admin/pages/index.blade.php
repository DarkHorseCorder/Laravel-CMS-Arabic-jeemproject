                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between p-0 ">
                            <ul class="nav nav-pills p-2 ml-auto">
                                <a class="nav-link {{ request()->routeIs('admin.pages.index')  ? 'active' : '' }}" href="{{ route('admin.pages.create') }}">
                                    Add page
                                </a>
                            </ul>
                        </div>
                                ...
                                <tbody>
                                    @foreach ($pages as $key => $page)
                                        <tr>
                                            ...
                                            <td>{{ $page->getTranslation('title', 'de') ?? "-" }}</td>
                                            <td>
        ...
        $( document ).ready(function() {
            $('#data-table').DataTable( {
                ...
                "ajax"       : {
                    ...
                    'url' : ('{{route('admin.pages.get')}}').replace('&amp;', '&'),
                    'dataSrc':""
                },
                "columns"   : [
                    {
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row) {
                            ...
                            var html =  '<div class="btn-group" role="group">';
                                ...
                                html += '    <a href="'+('{{route('admin.pages.edit', ':slug')}}').replace(':slug', data)+'" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></a>';
                                html += '    <form action="'+('{{route('admin.pages.destroy', ':slug')}}').replace(':slug', data)+'" method="POST" onsubmit="return confirm('+'Are you sure'+');">';
