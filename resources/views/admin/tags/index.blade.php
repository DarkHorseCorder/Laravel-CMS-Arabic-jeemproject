                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between p-0 ">
                            <ul class="nav nav-pills p-2 ml-auto">
                                <a class="nav-link {{ request()->routeIs('admin.tags.index')  ? 'active' : '' }}" href="{{ route('admin.tags.create') }}">
                                    Add Tag
                                </a>
                            </ul>
                        </div>
                        <div class="card-body table-responsive">
                            <table id="data-table" class="table table-striped table-hover datatable">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $tag->getTranslation('title', 'ar') ?? "-" }}</td>
                                        <td>{{ $tag->getTranslation('title', 'en') ?? "-" }}</td>
                                        <td>{{ $tag->getTranslation('title', 'de') ?? "-" }}</td>
                                        <td>
        ...
        $( document ).ready(function() {
            $('#data-table').DataTable( {
                ...
                "ajax"       : {
                    ...
                    'url' : ('{{route('admin.tags.get')}}').replace('&amp;', '&'),
                    'dataSrc':""
                },
                "columns"   : [
                    {
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row) {
                            ...
                            var html =  '<div class="btn-group" role="group">';
                                ...
                                html += '    <a href="'+('{{route('admin.tags.edit', ':slug')}}').replace(':slug', data)+'" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></a>';
                                html += '    <form action="'+('{{route('admin.tags.destroy', ':slug')}}').replace(':slug', data)+'" method="POST" onsubmit="return confirm('+'Are you sure'+');">';
