        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.posts.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('author_id') ? 'is-invalid' : '' }}" name="author_id" id="author" >
                        @foreach ($authors as $author)
                            <option value="{{$author->id }}" {{ old('author_id') ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple >
                        @foreach($categories as $id => $category)
                            <option value="{{$category->id }}" {{ in_array($id, old('categories', [])) ? 'selected' : '' }}>
                                {{ $category->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple >
                        @foreach($tags as $id => $tag)
                            <option value="{{$tag->id }}" {{ in_array($id, old('tags', [])) ? 'selected' : '' }}>
                                {{ $tag->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                @foreach(config('translatable.locales') as $locale)
                    <div class="border p-3 my-2">
                            ...
                            <div class="form-group">
                                ...
                                <textarea rows="2" class="form-control {{ $errors->has('body') ? 'is-invalid' : '' }}" name="body[{{ $locale }}]" id="body_{{ $locale }}">{{ old('body.'.$locale, '') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <button type="submit" class="btn btn-success" name="action" value="saveAsDraft" >
                        {{ trans('global.saveAsDraft') }}
                    </button>
