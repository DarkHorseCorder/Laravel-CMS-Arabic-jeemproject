                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between p-0 ">
                            <ul class="nav nav-pills p-2 ml-auto">
                                <a class="nav-link {{ request()->routeIs('admin.menus.index')  ? 'active' : '' }}" href="{{ route('admin.menus.create') }}">
                                    Add Menu
                                </a>
                            </ul>
                        </div>
                            ...
                            <table id="data-table" class="table table-striped table-hover datatable">
                                ...
                                <tbody>
                                    @foreach ($menus as $key => $menu)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $menu->getTranslation('title', 'ar') ?? "-" }}</td>
                                            <td>{{ $menu->getTranslation('title', 'en') ?? "-" }}</td>
                                            <td>{{ $menu->getTranslation('title', 'de') ?? "-" }}</td>
                                            <td>
        ...
        $( document ).ready(function() {
            $('#data-table').DataTable( {
                ...
                "ajax"       : {
                    ...
                    'url' : ('{{route('admin.menus.get')}}').replace('&amp;', '&'),
                    'dataSrc':""
                },
                "columns"   : [
                    {
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row) {
                            ...
                            var html =  '<div class="btn-group" role="group">';
                                ...
                                html += '    <a href="'+('{{route('admin.menus.edit', ':slug')}}').replace(':slug', data)+'" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></a>';
                                html += '    <form action="'+('{{route('admin.menus.destroy', ':slug')}}').replace(':slug', data)+'" method="POST" onsubmit="return confirm('+'Are you sure'+');">';
